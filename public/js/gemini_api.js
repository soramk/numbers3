...
以下はその「最適位相パラメータ」の推移データ（直近100回）です：
${phaseStr}
...
1. 【位相パターンの深層分析（直近100回）】
   ...
   - 特に直近20回の位相変化率を計算し、次回の位相を予測してください
...
5. 【3桁コンボパターンの詳細分析】
   - topCombos から頻出パターンを抽出し、それらが直近100回でどのように出現しているかを分析
   - 直近20回で出現した3桁コンボと、過去の頻出コンボを比較し、再出現の可能性を評価
   - 各桁の数字の組み合わせ（例: 百の位が1のとき、十の位と一の位の組み合わせ）を分析
   - 連続出現パターン（例: 同じ数字が連続する、等差数列など）を検出

6. 【コンフォーマル予測による信頼度校正】
   - 過去100回のデータを使って、仮想的に「候補集合」を提示した場合の的中率を逆算
...
7. 【異常検知とアウトライア分析】
   - 直近100回のデータから、統計的に異常なパターン（外れ値）を検出
   - 異常パターンの後に続く数字の傾向を分析し、異常後の反動を予測
   - 位相の急変点と実際の数字の変化の関係を分析

8. 【統合予測の生成】
   上記1〜7の分析結果を統合し、「次回の当選が統計的に期待される3桁の数字」を<strong>3パターン</strong>提示してください。
