...
### 1. 🔄 リセットボタン追加
- 数字検索・日付検索フォームに「リセット」ボタンを追加し、選択状態と検索結果をクリアできるようにする。

### 2. 🐛 ボックス検索チェックボックス再修正
- チェックボックスがまだ押しにくいという報告あり。`z-index` の確認や、`label` の入れ子構造を確実に修正する。

### 3. [NEW] 📊 ROIランキング表示機能
- **UI**: シミュレーションセクションに「ROIランキングを表示」ボタンを追加。
- **ロジック**: 
    - 過去の全出現数字について、「毎回その数字を買い続けた場合」の収支（ROI）を計算。
    - ストレート・ボックスそれぞれの収支を考慮（今回は簡易的にストレート狙いで計算するか、ボックス狙いか選択可能にするか、あるいはストレート全通りのランキングにする）。
    - ROIが高い順にTOP10をモーダルまたはリストで表示する。


### 4. 📐 index.html 幅修正
- メインコンテンツの幅が狭い問題を修正（`md:ml-64` や `container` クラスの適用範囲を見直し）。
